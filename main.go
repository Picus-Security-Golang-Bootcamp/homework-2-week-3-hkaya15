...
import (
	...
	"github.com/hkaya15/PicusSecurity/Week_3_Homework/args"
...
func main() {
	...
	case "list":
		flag.Parse()
		if len(os.Args) == 2 {
			res, err := args.NewFuncCommand(list).GetList(book.Book{})
			if err != nil {
				fmt.Println(err)
			}
	...
	case "search":
		...
		if len(os.Args) > 2 {
			...
			res, err := args.NewFuncCommand(search).SearchByName(book.Book{}, val)
			if err != nil {
				fmt.Println(err)
			}
			if len(res) > 0 {
	...
	case "get":
		...
		if *id == 0 {
			...
			v, err := args.NewFuncCommand(get).SearchById(book.Book{}, *id)
			if err != nil {
				fmt.Println(err)
			}
			if v.BookID == 0 {
	...
	case "delete":
		...
		if *deletedId == 0 {
			...
			err := args.NewFuncCommand(delete).DeleteById(book.Book{}, *deletedId)
			if err != nil {
				fmt.Println(err)
			}
		}
	...
	case "buy":
		...
		if *bookId == 0 {
			...
			if *quantity <= 0 {
				...
				err := args.NewFuncCommand(buy).BuyById(book.Book{}, *bookId, *quantity)
				if err != nil {
					fmt.Println(err)
				}
			}
